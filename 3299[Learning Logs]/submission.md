...
```text
โจทย์ให้จำลองการปลูกดอกไม้ลงในตารางสี่เหลี่ยมจัตุรัส โดยเริ่มจากมุมแล้วไล่ปลูกเป็นแถบแนวทแยงที่มีความหนา L ช่อง ไปเรื่อยๆ จนครบ N ช่อง แล้วถามว่าช่องสุดท้ายที่เราปลูกนั้นไปตกอยู่ในแถบที่เท่าไหร่

- Input: รับค่า L ความหนาของแถบ และ N จำนวนช่องที่ปลูก
- Output: พิมพ์ตัวเลขแสดงหมายเลขแถบของช่องสุดท้าย
```
...
```text
ตอนแรกคิดว่าจะลองจำลองการเดินช่องตารางทีละช่องตามแนวทแยงเพื่อดูว่ามันวิ่งไปถึงไหน แต่พอนึกถึงตอนที่ N มีค่าเยอะๆ น่าจะทำให้โปรแกรมทำงานช้า เลยเปลี่ยนมามองภาพรวมเป็นรูปสามเหลี่ยมสะสมแทน ว่าถ้าขยายแนวทแยงไปเรื่อยๆ พื้นที่สามเหลี่ยมจะครอบคลุมจำนวนช่องสะสมได้เท่าไหร่ แล้วค่อยเทียบหาแถบ L อีกที
```
...
```text
ใช้วิธีคำนวณทางคณิตศาสตร์โดยมองพื้นที่เป็นรูปสามเหลี่ยมแนวทแยง ระยะสะสม X จะทำให้ได้จำนวนช่องรวมเท่ากับ X(X+1)/2 เราเลยวนลูปหาค่า X น้อยที่สุดที่ทำให้ X(X+1) >= 2N จากนั้นค่อยเอาค่า X ไปหารด้วยความหนา L แล้วปัดเศษขึ้น เพื่อแปลงเป็นหมายเลขแถบ k ที่ต้องการ
```
...
```text
ทดสอบตามตัวอย่างแรกในโจทย์ เช็คความถูกต้องเบื้องต้น
```
...
```text
ทดสอบตัวอย่างที่สองในโจทย์ ที่ขนาดแถบ L หนาขึ้น L = 3
```
...
```text
ทดสอบกรณีเริ่มต้นที่น้อยที่สุด N = 1 ว่าคำตอบถูกต้องตั้งแต่ช่องแรกหรือไม่
```
