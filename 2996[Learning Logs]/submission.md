...
```text
โปรแกรมรับค่า String มาแล้วนำเทคนิค slice มาใช้ในโจทย์นี้ [start :stop :step] ผมเลือกที่จะใ่ส่เป็น [::-1] หรือ [0:0:-1] หมายถึงการอ่านตัวอักษรแบบย้อนกลับและใช้ .lower() ร่วมด้วเนื่องจากใน Testcase จะเห็นได้่ว่าโจทย์ไม่สนว่าจะเป็น Uppercase หรือ Lowercase แต่เมื่อแสดงผลออกมาต้องเป็น Lowercase
```
...
```text
แผนการแรกเลือกที่จะใช้เทคนิค slice
```
...
```text
ใช้เทคนิค slice คู่กับ .lower() เนื่องจาก Testcase ที่ออกมาเป็น lowercase ทั้งหมด
```
